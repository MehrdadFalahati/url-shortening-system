...
public class UrlShortenerMappingRepositoryImpl implements UrlShortenerMappingRepository {
    ...
    @Override
    public List<UrlShortenerMapping> findUnchangedDataInYear(ZonedDateTime start) {
        return urlShortenerMappingJpaRepository.findUnchangedDataInYear(start.toInstant())
                .stream()
                .map(urlShortenerDataAccessMapper::entityToUrlShortenerMapping)
                .toList();
    }
    ...
    @Override
    public void deleteAll(List<UrlShortenerMapping> urlShortenerMappings) {
        urlShortenerMappingJpaRepository.deleteAllByIdInBatch(urlShortenerMappings.stream().map(u -> u.getId().getValue()).toList());
    }
}
